...
var mylist = [24, 235, 5, 325, 253, 235, 322, 11, 79];
var ark = 'btv1b531212347';
var map;
...
function main(init) {
	...
	var leaflet = 'f1';
	var rot = 5;
	var imageUrl = 'https://gallica.bnf.fr/iiif/ark:/12148/' + ark + '/' + leaflet + '/full/full/' + rot + '/native.jpg';
	var imageBonds = [[48.822699, 2.254281], [48.89659, 2.411311]];
	...
	var level_paris={
		...
		//from BackThenMaps
		backthen_paris:layerImage(imageUrl, imageBonds)
	};
	...
	if (init){
		map = L.map('map', {
		    ...
		    zoom: 14,
		    layers: [level_paris.backthen_paris],
	...
	function updateLayers(level) {
	    ...
	    map.setView([48.855426, 2.345846], 14);
	  	baseLayers = {
	  	    ...
	  	    "Etat-Major Map 10K, 1840" : level_paris.ign_em10,
	  	    ...
	  	    "Base map": level_paris.backthen_paris

	  	}
	  	
		groupedOverlays = {
			"Overlays" : {
			"Wired road network": level_paris.rues_vasserot,
			"Vectorized blocks": level_paris.rues_verniquet,
			"Land attribution prediction": level_paris.rues_jacoubet,
			"Map deformation": level_paris.rues_poubelle
			}
...
var requestURL = 'https://rpetitpierre.github.io/assets/data/df_website.json';
...
function selectYearUpdate() {
	//document.getElementById("selectMap").reset();

	var selectedYear = document.getElementById("selectYear").selectedIndex + 1759;
	...
	request.onload = function() {
	  ...
	  (function() {
		  for (var i = 0; i < subset.length; i++) {
		    ...
		    var textNode = document.createTextNode(subset[i]['title'].slice(0, 80) + ' ...')
...
function selectMapUpdate() {
	...
	request.onload = function() {
	  var jsonObject = request.response;
	  selectedMap = jsonObject[document.getElementById("selectMap").selectedIndex]['ark'];
